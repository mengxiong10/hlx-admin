<template>
  <div class="app-container" style="padding: 1px;">
    ...
    <el-table v-loading="loading" :data="data" border size="small" style="width: 100%;">
      <el-table-column show-overflow-tooltip align="center" prop="qsTextbook.qsName" label="课程名称" />
      ...
      <el-table-column v-if="checkPermission(['ADMIN','QSTEXTBOOKTWO_ALL','QSTEXTBOOKTWO_EDIT','QSTEXTBOOKTWO_DELETE'])" label="操作" width="120px" align="center">
        <template slot-scope="scope">
          <edit v-permission="['ADMIN','QSTEXTBOOKTWO_ALL','QSTEXTBOOKTWO_EDIT']" :qs-textbook="qsTextbook" :data="scope.row" :sup_this="sup_this" />
          <el-popover v-permission="['ADMIN','QSTEXTBOOKTWO_ALL','QSTEXTBOOKTWO_DELETE']" :ref="scope.row.id" placement="top" width="180">
            <p>确定删除本条数据吗？</p>
...
export default {
  ...
  methods: {
    ...
    deleteCache() {
      ...
      delCache(this.qsTextbook.id).then(res => {
        this.$notify({
          title: '清除成功',
          type: 'success',
          duration: 2500
