  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="data" border size="small" style="width: 100%">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="课程线上总得分:">
                  <span>{{
                    isEmpty(props.row.onlineAllScore) ? '0' : props.row.onlineAllScore
                  }}</span>
                </el-form-item>
              </el-col>
              <el-col v-if="false" :span="8">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="记录时间:">
                  <span>{{
                    isEmpty(props.row.updateTime) ? '无' : parseTime(props.row.updateTime)
                  }}</span>
                </el-form-item>
              </el-col>
            </el-row>
      ...
      <el-table-column align="center" show-overflow-tooltip prop="studyRate" label="本单元学完">
        <template slot-scope="scope"> {{ scope.row.studyRate }}% </template>
      </el-table-column>
      ...
      <el-table-column
        v-if="checkPermission(['ADMIN', 'QSSTUDY_ALL', 'QSSTUDY_EDIT', 'QSSTUDY_DELETE'])"
        label="操作"
        width="240px"
        align="center"
      >
        <template slot-scope="scope">
          <edit
            v-permission="['ADMIN', 'QSSTUDY_ALL', 'QSSTUDY_EDIT']"
            :data="scope.row"
            :sup_this="sup_this"
          />
          <studyitem
...
export default {
  ...
  methods: {
    ...
    beforeInit() {
      ...
      if (stuId !== '' && stuId !== null) {
        this.params['stuName'] = stuId
      }
      const qstextbookId = query.qstextbookName
      if (qstextbookId !== '' && qstextbookId !== null) {
        this.params['textbookName'] = qstextbookId
      }
      const qsTextbookTwoName = query.qsTextbookTwoName
      if (qsTextbookTwoName !== '' && qsTextbookTwoName !== null) {
        this.params['unitName'] = qsTextbookTwoName
      }
      const deptId = query.deptName
      if (deptId !== '' && deptId !== null) {
        this.params['deptName'] = deptId
      }
      const teachId = query.teachName
      if (teachId !== '' && teachId !== null) {
        this.params['teachName'] = teachId
      }
